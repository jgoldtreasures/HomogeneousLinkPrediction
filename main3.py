...
def get_top_k_recommendations(graph, sim, i, k):
    return sorted(filter(lambda x: i != x and graph[i, x] != 1, range(len(sim[i]))), key=lambda x: sim[i][x],
                  reverse=True)[0:k]
...
def local_methods(edge_list, method):
    ...
    train_vertices_set = get_vertices_set(
        train_list)  # Need this because we have to only consider target users who are present in this train_vertices_set
...
def main():
    # # default-case/ help if len(sys.argv) < 3 : print( "python link_prediction.py
    # <common_neighbors/jaccard/adamic_adar/preferential_attachment/katz/friendtns> data_file_path") exit(1)
    #
    # # Command line argument parsing
    # method = sys.argv[1].strip()
    # dataset_path = sys.argv[2].strip()
    # edge_list = get_edge_list(dataset_path)
    edge_list = get_edge_list("data/fb-pages-food.csv")
    method = 'adamic_adar'
    ...
    if method == "common_neighbors" or method == "jaccard" or method == "adamic_adar" or method == "preferential_attachment":
        ...
        print("python link_prediction.py <common_neighbors/jaccard/adamic_adar/preferential_attachment/katz/friendtns"
              "> data_file_path")
